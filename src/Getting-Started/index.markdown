...
SAP Concur is a travel and expense software solution that lets 25M business travelers book their travel, capture expense, submit expense reports, and more.  SAP Concur processes over 55 million transactions every year, representing $50 billion spend on T&E.

![](./assets/getting-started1.png)

## What can I do with SAP Concur APIs?

The **Travel/Itinerary API (v1.1)** lets you access a SAP Concur user's itinerary, including hotel/flight booking info.  As an example, you can mash-up the Itinerary API with a restaurant database/API to provide recommendations of places to eat near a SAP Concur user's hotel booking.

The **Expense API (v3.0)** allows you to get (and push) a SAP Concur user's expense information, including expense line items, their types (e.g. food, lodging), totals, and even receipt images.

Here's an example on how to get an Itinerary List of a SAP Concur user:

```shell
GET https://www.concursolutions.com/api/travel/trip/v1.1
...
You can interact with the API using an OpenAPI interface [here](/api-explorer/)

![](./assets/getting-started2.png)
...
## Ideas for a SAP Concur powered business travel app:
...
* [Challengepost](http://challengepost.com/software/built-with/concur) lists SAP Concur API integrations from previous hackathons.  Definitely worth checking out!
...
* [Create a SAP Concur sandbox](#sandbox)
* [Generate an access token](#token)
* [POST/GET dummy data to/from SAP Concur](#dummy)
* [Example apps in the App Center and previous hackathons](#example)

## <a name="sandbox"></a>Create a SAP Concur sandbox

Before you build your first application that integrates with SAP Concur, you will want to create your own SAP Concur company specifically for development and testing. We call this a developer sandbox, or simply sandbox for short. It is within the sandbox that you will make your application known to SAP Concur.
...
7. Click **Get Started**. You should now be on the **My SAP Concur** page of your sandbox.

For more detailed information go to [Set Up Developer Sandbox](/manage-apps/register.html) .

## <a name="token"></a>Generate an access token
...
After logging in to http://concursolutions.com, go to Administration -> Register Partner Application -> SAP Concur Partner Application (Modify).  We need the consumer key so we can call the endpoint that would return the access token.

![](./assets/getting-started3.png)

### Step 2: Call the access token request endpoint
...
Note that we already have `X-ConsumerKey` from Step 1.  To generate the `Authorization: Basic` value, you need to Base64-encode the login ID, colon and password such that `john_developer@hotmail.com:Travel&Expense$2012` becomes `am9obl9kZXZlbG90bWFpbC5jb206VHJhdmVsJkV4cGVuc2UkMjAxMg==`.  If you're not doing this programmatically yet, you can use this [nifty web tool](http://www.base64encode.org/) to generate that value (remember to choose Encode).
...
If the call is successful, you should get an XML response with a `<Token>` node.  That's your access token. We recommend that you use [Postman](http://www.getpostman.com/), a Chrome extension, to help you manage your API calls (not just SAP Concur ones).  Here's what the API calls look like in Postman:

![](./assets/getting-started4.png)

### Step 3: Call the APIs to pull expense report items
...
This would return an Expense Report response, with a field called `ID`.  We need this ID to extract the expense line items we created in the app earlier.  To liven things up a bit, let's use the [OpenAPI](https://www.concursolutions.com/api/docs/index.html#!/Entries) documentation of the "Entries" API to get the individual expense line items:

![](./assets/getting-started5.png)

We highlighted two things here, the (oval) field where you put in your access token, and the (rectangle) field where you put in the `ID` we got from the previous API call.  Note that we can do this same call in curl, or in any fashion you want.  OpenAPI just provides us a consolidated way to make the API calls.

To execute the call, click the "Try it out!" button.  You should get a response like this below:

![](./assets/getting-started6.png)
...
## <a name="dummy"></a>POST/GET dummy data to/from SAP Concur - the API or app

### Pushing dummy data to SAP Concur using the API

POST a dummy Itinerary to SAP Concur:

```shell
POST https://www.concursolutions.com/api/travel/trip/v1.1/
Authorization: OAuth {token}
...
<Itinerary xmlns="http://www.concursolutions.com/api/travel/trip/2010/06">
  <TripName>TechCrunch Disrupt SAP Concur</TripName>
...
POST Expense Report Header to SAP Concur:

```shell
POST https://www.concursolutions.com/api/expense/expensereport/v1.1/report
Authorization: OAuth {token}
Content-Type: application/xml
```
...
**Hack tip 1:** You can POST using JSON by setting the header to `Content-Type:application/json` and checking out a sample JSON body in our [OpenAPI](https://www.concursolutions.com/api/docs/index.html#!/Entries) doc.  Note that the examples here use v1.1, and those shown in OpenAPI use v3.0.
...
```shell
GET https://www.concursolutions.com/api/expense/expensereport/v2.0/Reports/?reportcountry=US
Authorization: OAuth {token}
Content-Type: Application/json
```

POST Expense Report Entry:

```shell
POST https://www.concursolutions.com/api/expense/expensereport/v1.1/report/B6F4FD62FB424911A3B8/entry
Authorization: OAuth {token}
Content-Type: application/xml
```
...
### Pushing dummy expense data to SAP Concur using the app

You can access the web version of SAP Concur at http://concursolutions.com/ or get the mobile app from [App Store](https://itunes.apple.com/us/app/concur-travel-receipts-expense/id335023774?mt=8) or [Google Play](https://play.google.com/store/apps/details?id=com.concur.breeze).
...
## <a name="example"></a>Example apps

All app integrations with SAP Concur can be found in the [App Center](https://www.concur.com/en-us/app-center).  Below are just two examples that would hopefully give you an idea on what to build:

* [Challengepost](http://challengepost.com/software/built-with/concur) lists SAP Concur API integrations from previous hackathons.  Definitely worth checking out!
* [TravelText](https://www.concur.com/en-us/partners/transaction-capture/traveltext) - TravelText allows you to text your expenses right into SAP Concur. No more paper receipts and Excel sheet mayhem!  Check out their video demo [here](https://www.youtube.com/watch?v=sxY_PO-QKZ0).
* [Trover](https://www.concur.com/en-us/app-center/listing/nDhf34TiiC9RCocFM2xViin5c/Trover) * it's travel photography + business travel integration in one great app. Quoting from this [article](https://skift.com/2013/07/25/travel-photography-app-trover-secures-2-5-million-in-funding-from-concur/), what they get for integrating with SAP Concur is *"being able to tap into SAP Concur’s experience, knowledge and “great visibility into travel patterns"*.
